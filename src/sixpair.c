...
void set_master(usb_dev_handle *devh, int itfnum, int mac[6], int pid) {
	...
	for ( itfnum=0; itfnum<cfg->bNumInterfaces; ++itfnum ) {
	  ...
	  for ( alt = itf->altsetting;
	    ...
	    if ( dev->descriptor.idVendor == VID &&
            (dev->descriptor.idProduct == PS3_PID || dev->descriptor.idProduct == PS4_PID || dev->descriptor.idProduct == NAV_PID) &&
